...
function renderOverlapTracker() {
  const eligibleResponses = getResponseEntries()
  ...
  const bestCount = Math.max(0, ...weekendCounts.map((weekend) => weekend.names.length));
  const bestWeekends = weekendCounts.filter((weekend) => weekend.names.length === bestCount && bestCount > 0);
  ...
  if (bestWeekends.length) {
    const labels = bestWeekends.map((weekend) => weekend.summary).join(", ");
    summary.textContent = `${labels}: works best for everyone.`;
  } else {
    summary.textContent = "No shared availability yet.";
  }
  ...
  weekendCounts.forEach(({ label, detail, names }) => {
    ...
    count.textContent = `${names.length} ${names.length === 1 ? "person" : "people"} available`;
